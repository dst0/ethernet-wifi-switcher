...
# Mock check_internet function with state tracking
...
test_initialization_with_internet() {
    ...
    output=$(check_internet_with_state_tracking "eth0" 0)
...
test_initialization_without_internet() {
    ...
    output=$(check_internet_with_state_tracking "eth0" 1)
...
test_recovery_from_failure() {
    ...
    check_internet_with_state_tracking "eth0" 1 >/dev/null
...
test_loss_of_internet() {
    ...
    check_internet_with_state_tracking "eth0" 0 >/dev/null
...
test_no_logging_on_same_state_success() {
    ...
    assert_equals "" "$output" "Should not log when state unchanged"

    teardown
}

# Test: No logging when state doesn't change (failed -> failed)
test_no_logging_on_same_state_failed() {
...
test_multiple_state_changes() {
    ...
    # Init with success
    output1=$(check_internet_with_state_tracking "eth0" 0)
    assert_contains "$output1" "is active and has internet" "First message should be initialization"

    # Lose internet
    output2=$(check_internet_with_state_tracking "eth0" 1)
    assert_contains "$output2" "was working before" "Second message should be loss"

    # Recover
    output3=$(check_internet_with_state_tracking "eth0" 0)
    assert_contains "$output3" "recovered from failure" "Third message should be recovery"

    # Lose again
    output4=$(check_internet_with_state_tracking "eth0" 1)
    assert_contains "$output4" "was working before" "Fourth message should be loss again"
